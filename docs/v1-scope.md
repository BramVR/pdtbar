...
Current v1 drill-down covers allocation/concentration, income/calendar, and big movers/freshness. Allocation starts with a first-class Portfolio allocation chart fed by core overview facts: total value, open holding count, top holdings, top-N concentration, sectors, asset types, and cash when present. The chart stays compact in the main menu; Detailed info opens the whole allocation overview plus individual holding drill-down rows. Deferred full-product facets include performance-vs-benchmark divergence, cash drag pressure, and corporate-action decisions; add them after the first three work.
