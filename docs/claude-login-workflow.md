...
When a first fetch requests distributions, X-ray holdings, calendar events, or dividends, failure of one of those optional facets preserves the holdings-backed snapshot, marks detail fill degraded, and stores a redacted diagnostic for Data Health. Unfetched X-ray data stays `nil`; the non-optional distribution, calendar, and dividend fields use their empty values and rely on the degraded outcome plus diagnostic to distinguish failure from a genuine empty result. A Claude/PDT setup outage skips later optional calls instead of repeating the same unavailable request.
