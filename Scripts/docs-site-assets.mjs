...
:root,:root[data-theme="light"]{
  ...
  --green:#598b4f;
  --blue:#3f6fa8;
  --soft:rgba(13,124,117,.11);
  --code:#0d1715;
  --code-border:#29423d;
  --shadow:0 18px 48px rgba(18,26,22,.12);
}
...
html{overflow-x:clip;scroll-behavior:smooth;scroll-padding-top:26px}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}*,*::before,*::after{animation:none!important;transition:none!important}}
body{margin:0;overflow-x:clip;background:radial-gradient(circle at 88% 12%,color-mix(in srgb,var(--gold) 18%,transparent),transparent 26rem),linear-gradient(135deg,var(--paper-2),transparent 34rem),var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,sans-serif;line-height:1.65;-webkit-font-smoothing:antialiased}
...
main{max-width:1220px;width:100%;padding:42px clamp(20px,5vw,72px) 84px;margin:0 auto}
.home-hero{display:grid;grid-template-columns:minmax(280px,.76fr) minmax(380px,1fr);column-gap:clamp(24px,4vw,52px);row-gap:24px;align-content:start;align-items:center;border-bottom:1px solid var(--line);padding:12px 0 34px;margin-bottom:30px}.home-hero h1{font-size:clamp(3rem,6vw,5.4rem);line-height:.96;margin:0 0 18px;max-width:9ch}.lede{font-size:1.17rem;max-width:58ch}.eyebrow{margin:0 0 10px;color:var(--accent);font-size:.72rem;text-transform:uppercase;letter-spacing:.11em;font-weight:800}
...
.hero-art{position:relative;display:flex;align-items:center;justify-content:center;min-height:320px;isolation:isolate}.hero-art::before{content:"";position:absolute;inset:7% -4% -6% 9%;border-radius:28px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 22%,transparent),color-mix(in srgb,var(--gold) 20%,transparent));filter:blur(24px);opacity:.64;z-index:-1}.pulse-art{display:block;width:100%;max-width:620px;aspect-ratio:1672/941;border-radius:18px;overflow:hidden;border:1px solid color-mix(in srgb,var(--paper) 76%,var(--line));background:var(--paper-2);box-shadow:0 30px 78px rgba(9,16,14,.26)}.pulse-art img{display:block;width:100%;height:100%;object-fit:cover}:root[data-theme="dark"] .pulse-art{border-color:rgba(255,255,255,.12);box-shadow:0 34px 84px rgba(0,0,0,.48)}
.feature-row{grid-column:1/-1;display:flex;gap:8px;flex-wrap:wrap;margin-top:0}.feature-pill{display:inline-flex;align-items:center;gap:7px;border:1px solid var(--line);border-radius:999px;padding:6px 11px;background:color-mix(in srgb,var(--paper) 88%,transparent);color:var(--text);font-size:.82rem;font-weight:650}.feature-pill:nth-child(2){border-color:color-mix(in srgb,var(--gold) 38%,var(--line))}.feature-pill:nth-child(3){border-color:color-mix(in srgb,var(--green) 38%,var(--line))}.feature-pill:nth-child(4){border-color:color-mix(in srgb,var(--blue) 38%,var(--line))}.feature-pill:hover{border-color:var(--accent);color:var(--accent);background:var(--soft);text-decoration:none}
...
@media(max-width:960px){.shell{display:block}.sidebar{position:fixed;inset:0 28% 0 0;max-width:330px;z-index:15;transform:translateX(-102%);transition:transform .2s ease;box-shadow:var(--shadow);pointer-events:none}.sidebar.open{transform:translateX(0);pointer-events:auto}.nav-toggle{display:block}.theme-float{display:inline-grid;position:fixed;top:14px;right:62px;z-index:20;width:40px;height:40px;background:var(--paper);color:var(--ink);box-shadow:var(--shadow)}main{padding:62px 18px 56px}.home-hero{display:block;min-height:auto}.hero-art{margin-top:28px;min-height:0}.pulse-art{max-width:560px}.feature-row{margin-top:14px}.doc-grid{display:block}.toc{display:none}h1{font-size:2rem}.home-hero h1{font-size:3.15rem}.doc :is(h2,h3,h4) .anchor{display:none}}
`;
}
...
export function pulseArtSvg(src = "pdtbar-hero.png") {
  return `<picture class="pulse-art">
    <img src="${src}" alt="PDTBar portfolio pulse interface preview" width="1672" height="941" loading="eager" fetchpriority="high">
  </picture>`;
}
