...
classify_path() {
  ...
  case "$path" in
    *.md|docs/*.png|docs/*.jpg|docs/*.jpeg|docs/*.webp|docs/*.ico|docs/*.svg|docs/**/fixtures/*.json)
      ;;
    *)
      require_macos_tests "$path" "not covered by portable docs checks"
      ;;
  esac
}

invalid_row=false
while IFS=$'\t' read -r row_status first_path second_path extra_path \
  || [[ -n "${row_status:-}${first_path:-}${second_path:-}${extra_path:-}" ]]
do
  [[ -z "${row_status}${first_path:-}${second_path:-}${extra_path:-}" ]] && continue

  case "$row_status" in
    R*|C*)
      if ! [[ "$row_status" =~ ^[RC][0-9]{1,3}$ ]] \
        || ((10#${row_status:1} > 100)) \
...
if [[ "$macos_tests" == true ]]; then
  ...
  summary_reason="docs-only changes covered by portable checks"
fi
...
if [[ "$macos_tests" == true ]]; then
  ...
  printf 'Skipping macOS Swift tests for docs-only changes covered by portable checks.\n'
fi
